...
- ✅ 홀드 기능 (블록 저장 및 교환, 한 턴에 한 번 사용 가능)
- ✅ 콤보 시스템 (연속 라인 클리어 시 보너스 점수 및 화려한 효과)
...
- ✅ 게임 오버 감지
- ✅ 부드러운 애니메이션 효과
- ✅ 다양한 사운드 효과 (이동, 회전, 착지, 라인 클리어, 레벨업, 콤보 등)
- ✅ 음소거 토글 기능
...
### 콤보 시스템 🔥
- **연속 라인 클리어**로 콤보 증가
- **콤보 보너스**: (콤보 수 - 1) × 50점 × 레벨
- **시각적 효과**:
  - 2-4 콤보: 노란색 표시
  - 5-9 콤보: 주황색 표시 + 화면 플래시
  - 10+ 콤보: 보라색 표시 + 화면 플래시 + 흔들림
- **콤보 리셋**: 라인을 클리어하지 못한 채 블록이 착지하면 콤보가 0으로 초기화
- **콤보 사운드**: 콤보 수에 따라 점점 더 화려한 사운드 재생
...
게임에는 다양한 효과음이 추가되어 있습니다:

- **이동 사운드**: 블록을 좌우로 이동할 때
- **회전 사운드**: 블록을 회전할 때
- **착지 사운드**: 블록이 바닥이나 다른 블록에 닿을 때
- **라인 클리어**: 줄을 완성했을 때 (1-3줄)
- **테트리스**: 4줄을 한 번에 클리어했을 때 (특별한 사운드!)
- **콤보**: 연속으로 라인을 클리어할 때 (콤보 수에 따라 소리가 점점 화려해짐!)
- **레벨업**: 새로운 레벨에 도달했을 때
- **홀드**: 블록을 홀드 슬롯에 저장/교환할 때
- **하드 드롭**: Space 키로 블록을 즉시 떨어뜨릴 때
- **게임 오버**: 게임이 끝났을 때
- **메뉴 선택**: 버튼을 클릭할 때

게임 화면 오른쪽 하단의 **🔊 사운드** 버튼을 클릭하여 언제든지 음소거할 수 있습니다.
...
3. **홀드 기능**: 블록을 저장하고 교환하여 전략적 플레이 가능
4. **콤보 시스템**: 연속 라인 클리어로 보너스 점수 및 화려한 시각 효과
5. **라인 클리어 애니메이션**: 줄이 완성되면 빠르게 깜빡이는 효과
6. **레벨 업 효과**: 레벨이 오르면 화면 플래시 효과
...
5. **사운드 시스템**: Web Audio API를 활용한 실시간 효과음 생성
6. **음소거 토글**: 게임 중 간편한 사운드 ON/OFF
...
### 키 입력 딜레이 조정
```javascript
this.moveDelay = 100; // 좌우 이동 딜레이 (밀리초) - 빠른 반응
this.softDropDelay = 50; // 소프트 드롭 딜레이 (밀리초) - 균형잡힌 속도
```
